...
async function setupCodex(
  ...
  try {
    ...
    const trackerProjectDir = path.resolve(options.cwd ?? process.cwd());
    const collectorCommand = `cd ${shellQuote(trackerProjectDir)} && pnpm cli collect --agent codex --quiet`;
    ...
    const helperScript = createHelperScript(trackerProjectDir);
    ...
    return {
      ...
      lines: [
        ...
        "3. Run `pnpm cli ingest` and then `pnpm cli summary --since 7d`.",
      ],
    };
  } catch (error) {
    return {
...
function createHelperScript(trackerProjectDir: string): string {
  return `#!/usr/bin/env sh
# Generated by himan-tracker. This script must never block Codex.
TRACKER_PROJECT_DIR=${shellQuote(trackerProjectDir)}
